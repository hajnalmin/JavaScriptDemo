(function(window){
		...
		init:function(selector,context){
			...
			if(!isString(selector)){
				...
				if(/<(\w+).*?>(.*?)(<\/\1>)/[attr_test](selector)){
					...
					if(selector)self[attr_push][attr_apply](self,toArray(self[attr_context][attr_querySelectorAll](selector)))
				}
    ...
    (function(){
    	...
    	nick[attr_fn][attr_extend]({
    		...
    		//获取当前元素的索引值，可以指定选择器对匹配的元素进行计数
    		//Access to the current element index value, you can specify a selector to the count of the matched elements
    		...
    		index:function(selector){
    			...
    			//如果要匹配的选择器不是字符串则从当前所匹配的元素中查找与所指定元素相等的并返回在当前所匹配元素中的索引位置
    			//If you want to match the selector is not a string from the current search with the specified element in the matched element equivalent and returns the index of the matched elements in the current position
    			
    			if(selector && !isString(selector)){
    				
    				//遍历当前所匹配的元素
    				//Through the current of the matched elements
    				
    				return each(this,function(i){
    					
    					//如果当前元素与指定的元素相等则返回索引值
    					//If the current element with the index value equal to the specified elements
    					
    					if((selector[0] || selector) == this) return i
    					
    				})
    				
    			}else{
    			
	    			//循环查找上一个兄弟节点，若找不到或找的节点等于自己循环终止
	    			//Loop through nodes on a brother, if can't find or find nodes is equal to the loop terminates
	    			
	    			do{
	    				//若当前查找的元素不等于自己且符合指定的选择器则索引值加1
	    				//If the elements of the current search is not equal to yourself, and conform to the specified index of the selector is value plus one
	    				
	    				if(prev!=$this && selector ? prev[attr_matchesSelector](selector) : real)index++;
	
	    				prev = prev[attr_previousElementSibling];
    		...
    		//对当前所匹配的元素进行再次匹配，不符合的元素将从结果中删除
    		//To match the current element in the match again, is not in conformity with the elements will be deleted from the results
    		...
    		filter:function(selector){
    			...
    			//遍历当前匹配到的元素
    			//Traverse the current matching to the elements
    			
    			each(this,function(){
    				
    				$this = this;
    				
    				//若当前元素成功与指定的选择器匹配则添加到数组中
    				//If the current element specified selectors matching success is added to the array
    				
    				if($this[attr_matchesSelector] && $this[attr_matchesSelector](selector))elements[attr_push]($this)
    ...
    (function(){
		...
		var unitCompletion = function(attr,value){
			...
			if(/^[-\+]?\d+\.*\d*$/[attr_test](value)) value += units[attr] === undefined ? 'px' : units[attr];
        ...
        var css = function(myself,attr,value){
        	...
        	//将样式转换成字符串
        	//The style is transformed into a string
        	
        	cssText = cssToText(css);
        	...
        	return bind(myself,function(){
        		...
        		$this[attr_style][attr_cssText] += cssText;
        		
        		//遍历样式若样式属性值为空或者是窗口对象则删除当前的样式，删除后将不在style中出现
        		//Traversal is empty or if a style style attribute value is the window object is deleted the current style, will not appear in style after deletion
        		
        		each(css,function(i){
        			
        			if(vacancy === this) $this[attr_style][i] = vacancy
        ...
        var eventsCallback = function($this,type,event,allEvents,x,y){
    		...
    		//List traversal process after the event
   			each(events,function(){
   				...
   				each(self[1],function(){
   					...
   					this[attr_call](self[0],event,x,y) !== fake || event[attr_preventDefault]();
   		...
   		var gestureEvent = function($this,attr_subline){
    		...
    		$this[attr_addEventListener](attr_touch+attr_move,function(event){
    			...
    			//获取移动范围在滑动事件回调函数中可以使用，并且根据xy值是否大于0可以判断移动方向
    			//Get moving range in the sliding event callback function can be used, and according to the y value greater than 0 can determine direction of movement
    			...
    			if(toggle) eventsCallback($this,attr_slip,event,[$this[attr_events][attr_slip],$this[attr_events][attr_slip+attr_subline]],moveX,moveY)
    		...
    		$this[attr_addEventListener](attr_touch+attr_end,function(event){
    			...
    			touchLongTime = $this[attr_touchLongTime] || 300;
   		...
   		//将移动端判断条件添加到nick属性上
   		//Add the mobile end judgment conditions to the Nick properties
		...
		(function(){
			...
			var parseAnimationOptions = function(style,options){
	   		...
	   		var completeAnimationQueue = function($this,attr,animationRules){
	   			...
	   			//将执行过的回调函数清除
	   			//Will perform a callback function to clear
	   			
	   			$this[attr_animationCallback] = fake;
	   			
	   			//动画完成之后将动画属性设置为未定义以免动画属性的存在影响元素的样式
	   			//Animation has completed the animation attribute is set to undefined so as to avoid the existence of the animation properties affect the style of the element
	   			
	   			css([$this],attr_animation,vacancy+empty);
	   		...
	   		var clearAnimationQueue = function(myself){
	   			...
	   			return bind(myself,function(){
	   				...
	   				if($this[attr_animationQueue]){
	   					...
	   					//清除队列时读取当前动画规则，遍历当前动画规则中的属性列表，根据这些属性名称将当前对应的样式保存到style中去
	   					//Remove queue when read the current animation rules, rules of traverse the current animation attributes in the list, according to the attribute name to save the current corresponding style to style
	   					
	   					each(getAnimationRules($this[attr_lastAnimationName])[attr_split](';'),function(){
	   						
	   						//将样式规则以:号拆分得到一个数组，第一个元素为属性名，第二个元素为属性值 
	   						//Will split down style rules to: get an array, the first element is the property name, the second element of attribute values
	   						
	   						self = this[attr_split](':');
	   						
	   						//设置style行内样式为当前样式以保存动画执行的状态不退到初始状态
	   					...
	   					//删除动画样式属性
	   					//Delete the animation style properties
	   					
	   					$this[attr_style][attr_animation] = vacancy;
